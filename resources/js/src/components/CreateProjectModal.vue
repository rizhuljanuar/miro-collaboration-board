...
async function submit(): Promise<void> {
  ...
  try {
    ...
    formError.value =
      error instanceof Error ? error.message : 'Terjadi kesalahan saat membuat project.';
  } finally {
  ...
  >
    ...
    >
      <div class="flex items-start justify-between gap-4">
        <div>
          <p class="text-sm font-bold uppercase tracking-[0.18em] text-blue-600">New workspace</p>
      ...
      <form class="mt-7 space-y-5" @submit.prevent="submit">
        <div>
          ...
          <p v-if="getFieldError('name')" id="project-name-error" class="mt-2 text-sm text-red-700">
            {{ getFieldError('name') }}
          </p>
        ...
        <div>
          ...
          <p class="mt-2 text-right text-xs text-slate-500">{{ description.length }} / 2000</p>
        </div>
